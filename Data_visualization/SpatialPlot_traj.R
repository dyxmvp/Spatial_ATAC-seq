SpatialPlot_traj <- function (object, group.by = NULL, features = NULL, images = NULL, 
  ...
  for (i in 1:ncols) {
    ...
    for (j in 1:length(x = features)) {
      ...
      if (is.null(x = group.by)) {
        ...
        plot <- plot + scale_fill_gradientn(name = features[j], 
                                            colours = ArchRPalettes$horizonExtra, oob = scales::squish) + theme(legend.position = "top") + 
          scale_alpha(range = alpha) + guides(alpha = FALSE)
      }
      else if (label) {
